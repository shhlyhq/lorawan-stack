...
func TestCSRF(t *testing.T) {
	...
	mSkipTrue := CSRF(skipTrue, authKey)
	mSkipFalse := CSRF(skipFalse, authKey)
	...
	t.Run("Allows access with valid CSRF token", func(t *testing.T) {
		...
		rec = httptest.NewRecorder()
		mSkipTrue(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {})).ServeHTTP(rec, r)
		res = rec.Result()
		a.So(res.StatusCode, should.Equal, http.StatusOK)
	})

	t.Run("Skip protection when skip check function returns true", func(t *testing.T) {
		r := httptest.NewRequest(http.MethodPost, "/", nil)
		rec := httptest.NewRecorder()
	...
	t.Run("Don't skip protection when skip check function returns false", func(t *testing.T) {
		r := httptest.NewRequest(http.MethodPost, "/", nil)
		rec := httptest.NewRecorder()
